  ...
  useEffect(() => {
    const fetchInvoices = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const response = await fetch("https://api.lizlyskincare.sbs/getInvoices.php", {
          headers: {
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    Overview of your clinic's sales performance
                  </p>
                </div>
...
<select
  ...
  className="w-full appearance-none bg-gray-50 border border-gray-200 text-gray-700 py-2 px-4 pr-8 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm"
>
  <option value="all">All Branches</option>
  {branches.map((branch) => (
    <option key={branch.id} value={branch.name}> {/* Use branch.name instead of branch.id */}
      {branch.name}
    </option>
  ))}
</select>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                ...
                <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-semibold text-gray-800 flex items-center">
                      <BarChart2 className="mr-2 text-emerald-600" size={18} />
                ...
                <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-semibold text-gray-800 flex items-center">
                      <FileText className="mr-2 text-emerald-600" size={18} />
                      Recent Invoices
                    </h3>
                  </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200 text-sm">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {currentInvoices.slice(0, 5).map((invoice) => (
                          <tr
                            key={`recent-${invoice.invoiceNumber}`}
                ...
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between">
                  ...
                  <div className="flex space-x-2">
                    <motion.button
                      ...
                      whileTap={{ scale: 0.97 }}
                    >
                      <FileText className="mr-1" size={14} /> Invoices
                    </motion.button>
                  </div>
                </div>
              </div>
